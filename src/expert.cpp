...
static QString convertToComment(const QString &s)
{
   if (s.isEmpty()) {
      ...
      return SA("# ") + s.trimmed().replace(SA("\n"), SA("\n# ")).replace(SA("# \n"), SA("#\n")) + SA("\n");
   }
}
   ...
   while (!child.isNull()) {
      ...
      if (setting.isEmpty() || IS_SUPPORTED(setting.toAscii())) {
         ...
         if (type == SA("bool")) {
            ...
            connect(boolOption, SIGNAL(changed()), SLOT(changed()));
         } else if (type == SA("string")) {
            ...
            connect(stringOption, SIGNAL(changed()), SLOT(changed()));
            ...
            connect(enumList, SIGNAL(changed()), SLOT(changed()));
         } else if (type == SA("int")) {
            InputInt *intOption =
               new InputInt(
               layout, row,
            ...
            connect(intOption, SIGNAL(changed()), SLOT(changed()));
         } else if (type == SA("list")) {
            ...
            connect(listOption, SIGNAL(changed()), SLOT(changed()));
